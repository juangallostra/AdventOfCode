...
def part2(data):
    ...
    * 9 has all horizontal segments, 0 doesn't
    ...
    combinations = [(val.split(' | ')[0].split(
    ...
    for combination in combinations:
        ...
        number_to_segments['1'] = [[l for l in val]
                                   for val in source if len(val) == 2][0]
        number_to_segments['4'] = [[l for l in val]
                                   for val in source if len(val) == 4][0]
        number_to_segments['7'] = [[l for l in val]
                                   for val in source if len(val) == 3][0]
        number_to_segments['8'] = [[l for l in val]
                                   for val in source if len(val) == 7][0]
        ...
        num_3 = [l for i, l in enumerate(seg_5) if len(set(l).intersection(set(
            seg_5[(i+1) % 3]))) == 4 and len(set(l).intersection(set(seg_5[(i+2) % 3]))) == 4][0]
        number_to_segments['3'] = num_3
        # get unique 6 segment combinations:
        seg_6 = [[l for l in val] for val in source if len(val) == 6]
        num_6 = [l for l in seg_6 if len(set(l).intersection(set(number_to_segments['1'][0]))) == 0 or len(
            set(l).intersection(set(number_to_segments['1'][1]))) == 0][0]
        number_to_segments['6'] = num_6
        # get top right segment:
        t_r = [a for a in number_to_segments['1']
               if a not in number_to_segments['6']][0]
        # 5 and 2
        seg_5_2 = [[l for l in val] for val in source if (len(val) == 5 and len(
            set([l for l in val]).union(set(number_to_segments['3']))) > 5)]
        ...
        horizontal_seg = [l for l in set(number_to_segments['5']).intersection(
            set(number_to_segments['2']))]
        # get 0 and 9
        seg_0_9 = [[l for l in val] for val in source if len(val) == 6 and len(
            set([l for l in val]).union(set(number_to_segments['6']))) > 6]
        num_0 = [i for i in seg_0_9 if not set(horizontal_seg).issubset(
            set(i).intersection(set(horizontal_seg)))][0]
        num_9 = [i for i in seg_0_9 if set(horizontal_seg).issubset(
            set(i).intersection(set(horizontal_seg)))][0]
