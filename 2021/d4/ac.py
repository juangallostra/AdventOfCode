...
class Board():
    ...
    def check_cols(self):
        cols = [[self.values[row_idx][col_idx].checked for row_idx in range(
            len(self.values))] for col_idx in range(len(self.values[0]))]
    ...
    def check_rows(self):
        # Check if any col has all cells set to True
        all_true = [idx for idx in range(len(self.values)) if all(
            [c.checked for c in self.values[idx]])]
        if len(all_true) > 0:
            return all_true[0]
        return -1
    ...
    def get_score(self, last_call):
        p1_score = sum(
            [c.value for row in self.values for c in row if not c.checked])
...
def part2(data):
    ...
    last_board = sorted([b for b in boards if b.has_won],
                        key=lambda x: x.winning_position)[-1]
